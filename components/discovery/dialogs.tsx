  ...
  return (
    <Dialog open={Boolean(kind)} onOpenChange={(open) => { if (!open) onClose() }}>
      <DialogContent className="glass-modal info-dialog">
        <span className="info-mark">{isAbout ? <BrandMark className="info-brand-mark" /> : <Keyboard className="size-8" aria-hidden="true" />}</span>
