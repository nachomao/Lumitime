    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-modal settings-dialog">
        ...
        <FieldGroup>
          ...
          <Field orientation="horizontal"><FieldContent><FieldLabel htmlFor="scenery-setting">沉浸壁纸</FieldLabel><FieldDescription>在通透玻璃之间，留一点自然</FieldDescription></FieldContent><Switch id="scenery-setting" checked={scenery} onCheckedChange={onSceneryChange} /></Field>
        </FieldGroup>
        <Separator />
  ...
  return (
    <Dialog open={Boolean(kind)} onOpenChange={(open) => { if (!open) onClose() }}>
      <DialogContent className="glass-modal info-dialog">
        <span className="info-mark">{isAbout ? <Compass className="size-8" /> : <Keyboard className="size-8" />}</span>
