...
import { ArrowUpRight, Check, ExternalLink, Hourglass, Keyboard, ShieldCheck, Sparkles } from 'lucide-react'
  ...
  return (
    <Dialog open={Boolean(kind)} onOpenChange={(open) => { if (!open) onClose() }}>
      <DialogContent className="glass-modal info-dialog">
        <span className="info-mark">{isAbout ? <Hourglass className="size-8" aria-hidden="true" /> : <Keyboard className="size-8" aria-hidden="true" />}</span>
